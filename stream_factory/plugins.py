...
class BlankInsertStreamPlugin(StreamPlugin):
    """流插件开发案例：每隔 ``interval`` 秒插入 ``duration`` 秒空白段（黑屏 + 静音）。

    展示如何自定义 ``StreamPlugin`` 的流级时间操作：覆盖 ``blanks()`` 返回空白段规则，
    pipeline 翻译为 ``drawbox``（视频盖黑）+ ``volume``（音频静音）滤镜，并触发重编码。
    ...
    def __init__(self, interval: float = 10.0, duration: float = 2.0):
    ...
    def blanks(self, source: StreamSource) -> List[BlankSegment]:
        return [BlankSegment(interval=self.interval, duration=self.duration)]
