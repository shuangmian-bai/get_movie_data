...
public class YunyMovieService implements MovieService {
    ...
    public List<Movie> searchMovies(String baseUrl, String keyword) {
        try {
            // 对关键词进行URL编码
            String encodedKeyword = java.net.URLEncoder.encode(keyword, java.nio.charset.StandardCharsets.UTF_8.toString());
            ...
            String infoText = HtmlParserUtil.select(doc, "#__nuxt > div > section > section > div.flex-1.flex.flex-col.overflow-y-auto > div.flex-1.pr-5 > div:nth-child(2) > div.search_result_info").text();
            ...
            int movieCount = HtmlParserUtil.select(doc, "div.search_result_list").select("a").size();
    ...
    public List<Movie> getMovies(String encodedKeyword, int page, String baseUrl) throws JsonProcessingException {
        ...
        // 发送HTTP请求
        String html = HttpClientUtil.sendGetRequest(url);
    ...
    public List<Movie.Episode> getEpisodes(String baseUrl, String playUrl) {
        try {
            ...
            Elements elements = HtmlParserUtil.select(doc, "#__nuxt > div > section > section > div.flex-1.flex.flex-col.overflow-y-auto > div.flex-1.pr-5 > div.video-detail-main > div.episode_box_main > div > a");
