"""非凡采集站（ffzy5.tv）插件主类、映射模板、抽象方法实现"""
...
class Ffzy5Plugin(MediaSourcePlugin):
    """非凡采集站（ffzy5.tv）站点插件。

    原始字段：title/href/cover/type/year/intro + episodes（name/url/index）。
    站点为苹果CMS，详情页内嵌 ffm3u8 线路，分集链接即为直接 m3u8 地址。
    """

    # ---- 站点元信息 ----
    base_url = "http://ffzy5.tv"
    source_name = "非凡采集站"
    source_desc = "非凡采集站（ffzy5.tv）：苹果CMS 站，影视搜索 / 详情 / m3u8 播放地址解析"
    ...
    async def _raw_search(self, key: str) -> List[Dict[str, Any]]:
        from media_source.plugins.ffzy5 import parser
