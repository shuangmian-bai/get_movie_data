"""红牛资源（hongniuziyuan.net）站点专属解析逻辑（原始数据提取）

站点为苹果CMS，搜索页是 JS 壳，站点前端实际调用 macapi1.com 的
JSON 数据接口（slug=hongniu），因此本插件直接复用该接口：
...
from media_source.plugins.hongniuzy.constants import (
    BASE_URL,
    HEADERS,
    abs_url,
    detail_api_url,
...
async def _fetch_json(url: str) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict) or data.get("code") != 1:
        logger.warning("hongniuzy 接口返回异常: %s", str(data)[:200])
        return {}
    return data
    ...
    if total_pages > 1:
        ...
        async def fetch_page(page: int) -> List[Dict[str, Any]]:
            try:
                ...
                logger.warning("hongniuzy 抓取第 %d 页失败: %s", page, exc)
                return []
