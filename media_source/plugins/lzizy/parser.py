"""量子采集站（lzizy.net）站点专属解析逻辑（原始数据提取）

站点为苹果CMS，但搜索页（``/lz/lz.html``）是 JS 壳，直接请求
``/index.php/vod/search.html`` 会被 WAF 拒绝（403）。站点前端实际调用
`macapi1.com` 的 JSON 数据接口，因此本插件直接复用该接口：
...
> 站点页面可能存在异常注入的重复片段；JSON 接口无此问题，分集仍按地址去重兜底。
...
from media_source.plugins.lzizy.constants import (
    BASE_URL,
    HEADERS,
    abs_url,
    detail_api_url,
...
# 详情页 / 详情链接中的 vod_id：/index.php/vod/detail/id/<id>.html
...
async def _fetch_json(url: str) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict) or data.get("code") != 1:
        logger.warning("lzizy 接口返回异常: %s", str(data)[:200])
        return {}
    return data
...
def _parse_episodes(record: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    ``vod_play_from`` / ``vod_play_url`` 可能用 ``$$$`` 分隔多条线路，
    优先取含 m3u8 的线路；无序号条目（如电影 ``HD国语``）index 置 0，
    由基类按位置自动补号。
    """
    play_froms = re.split(r"\$\$\$", record.get("vod_play_from") or "")
    play_urls = re.split(r"\$\$\$", record.get("vod_play_url") or "")

    # 优先 m3u8 线路，其次取第一条非空线路
    prefer = 0
    ...
    if total_pages > 1:
        ...
        async def fetch_page(page: int) -> List[Dict[str, Any]]:
            try:
                ...
                logger.warning("lzizy 抓取第 %d 页失败: %s", page, exc)
                return []
...
async def parse_play_url(play_url: str) -> Dict[str, Any]:
    ...
    详情接口返回的即为可直接播放的 m3u8 直链，无需再请求播放页，
    因此这里直接透传分集链接。
    """
