"""量子采集站（lzizy.net）插件主类、映射模板、抽象方法实现"""
...
class LzizyPlugin(MediaSourcePlugin):
    """量子采集站（lzizy.net）站点插件。

    原始字段：title/href/cover/type/year/intro + episodes（name/url/index）。
    站点为苹果CMS，搜索/详情复用站点前端的 JSON 数据接口，
    播放串内即为直接 m3u8 地址。
    """
    ...
    source_name = "量子采集站"
    source_desc = "量子采集站（lzizy.net）：苹果CMS 站，影视搜索 / 详情 / m3u8 播放地址解析"
    ...
    async def _raw_search(self, key: str) -> List[Dict[str, Any]]:
        from media_source.plugins.lzizy import parser
