"""鸭鸭资源库（yayazy.com）站点专属解析逻辑（原始数据提取）

站点为苹果CMS，前端实际调用 macapi1.com 的 JSON 数据接口
（slug=yaya），因此本插件直接复用该接口：
...
async def _fetch_json(url: str) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict) or data.get("code") != 1:
        logger.warning("yayazy 接口返回异常: %s", str(data)[:200])
        return {}
    return data
    ...
    if total_pages > 1:
        ...
        async def fetch_page(page: int) -> List[Dict[str, Any]]:
            try:
                ...
                logger.warning("yayazy 抓取第 %d 页失败: %s", page, exc)
                return []
...
async def parse_play_url(play_url: str) -> Dict[str, Any]:
    ...
    return {
        "play_url": url,
        "play_type": "m3u8",
        "headers": {"Referer": f"{DETAIL_HOST}/", "User-Agent": HEADERS["User-Agent"]},
    }
