"""茶杯狐（cupfox7.com）插件主类、映射模板、抽象方法实现"""
...
class CupfoxPlugin(MediaSourcePlugin):
    """茶杯狐（cupfox7.com）站点插件。

    原始字段：title/href/cover/type/year/intro + episodes（name/url/index）。
    站点为苹果CMS，播放地址为 m3u8/mp4，由播放页内嵌播放器变量提取。
    """

    # ---- 站点元信息 ----
    base_url = "https://www.cupfox7.com"
    source_name = "茶杯狐"
    source_desc = "茶杯狐（cupfox7.com）：苹果CMS 站，影视搜索 / 详情 / m3u8 播放地址解析"
    ...
    async def _raw_search(self, key: str) -> List[Dict[str, Any]]:
        from media_source.plugins.cupfox import parser
