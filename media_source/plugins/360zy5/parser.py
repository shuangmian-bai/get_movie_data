"""360 资源站（360zy5.com）站点专属解析逻辑（原始数据提取）

站点为苹果CMS，官方采集接口 ``api.php/provide/vod/`` 支持关键词搜索，
搜索与详情均返回完整字段（含 ``vod_play_url`` 播放串）：
...
async def _fetch_json(url: str) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict) or data.get("code") != 1:
        logger.warning("360zy5 接口返回异常: %s", str(data)[:200])
        return {}
    return data
    ...
    if total_pages > 1:
        ...
        async def fetch_page(page: int) -> List[Dict[str, Any]]:
            try:
                ...
                logger.warning("360zy5 抓取第 %d 页失败: %s", page, exc)
                return []
