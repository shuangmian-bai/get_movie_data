...
- **`PluginManager`**（`plugin_manager.py`）在导入时通过 `pkgutil` 扫描 `media_source.plugins/*`，跳过 `template` 与 `_deprecated` 目录。全局单例 `plugin_manager` 在模块导入时创建并扫描（`from media_source import plugin_manager`）。关键方法：`get_supported_sources()`、`get_plugin_instance(base_url)`、异步 `batch_search(key, base_urls=[], max_concurrency=None)`（信号量限流 + 单插件异常隔离）。
...
- 源视频缓存（`video_cache.py`）：ffmpeg 拉流前先把上游源缓存到本地（保留 HLS 分片结构 + AES-128 key + fMP4 init 分片 / mp4 直链），`factory.create_stream` 自动 `ensure_source` 命中后改读本地；用**长驻 httpx 连接池**复用 TCP、per-key 锁**并发去重**（同源只下载一次）、TTL 过期惰性重下；AES-128 加密分片也一并缓存（key/分片本地化 + URI 重写，ffmpeg 加 `-allowed_extensions ALL` 离线解密）；m3u8 默认**边下边推**（首个分片就绪即写 live `index.m3u8` 返回、后台按序追加剩余分片，ffmpeg 边读边推，`VIDEO_CACHE_STREAMING=0` 回退全量下载）；仅 Master 嵌套过深 / 首分片、key、init 下载失败或就绪超时**降级直连**，中间分片失败重试后跳过。缓存根 `{项目根}/cache/video_cache/`（`VIDEO_CACHE_ROOT`/`VIDEO_CACHE_TTL`/`VIDEO_CACHE_CONCURRENCY`/`VIDEO_CACHE_STREAMING`/`VIDEO_CACHE_SEGMENT_RETRY` 可配）。
