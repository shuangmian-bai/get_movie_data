...
- 源视频缓存（`video_cache.py`）：ffmpeg 拉流前先把上游源缓存到本地（保留 HLS 分片结构 / mp4 直链），`factory.create_stream` 自动 `ensure_source` 命中后改读本地；用**长驻 httpx 连接池**复用 TCP、per-key 锁**并发去重**（同源只下载一次）、TTL 过期惰性重下；AES-128 加密分片 / Master 嵌套过深 / 下载失败均**降级直连**。缓存根 `{项目根}/cache/video_cache/`（`VIDEO_CACHE_ROOT`/`VIDEO_CACHE_TTL`/`VIDEO_CACHE_CONCURRENCY` 可配）。
